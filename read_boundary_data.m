function [bound,bound_vals,bound_amplitudes] = read_boundary_data(fid,basic_data)
...
bound(1:max_ndof*nnode)=0;
bound_vals(1:max_ndof*nnode) = 0;
bound_amplitudes(1:max_ndof*nnode) = 0;
tl=fgetl(fid);
while tl ~= -1
    ...
    bound(dof_nums)=1;
    bound_vals(dof_nums)=val_dum;
    bound_amplitudes(dof_nums)=amp_num;
    tl=fgetl(fid);
end
